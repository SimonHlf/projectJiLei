...
<body>
		...
		<ul>
			<%-- <li>
				...
				<div class="inpDiv" tapmode onclick="selectBank()">
					<input id="selBankInp" type="text" value="${frontUser.khf }" placeholder="请选择开户行">
					<!-- disabled -->
			...
			<li>
				...
				<div class="inpDiv">
                    <input id="selBankLocInp" type="text" value="${frontUser.province }" readonly placeholder="请选择开户地址">
                </div>
			</li> --%>
			<li>
				<div class="txtDiv">微信昵称：</div>
				<div class="inpDiv">
					<input id="personNameInp" value="${frontUser.realName }" type="text" readonly="readonly"><!-- onblur="checkNameNum()" -->
				</div>
			</li>
			<li>
				<div class="txtDiv">
					微<span class="twoBlank"></span>信<span class="twoBlank"></span>号：
				</div>
				<div class="inpDiv">
					<input id="bankNumInp" value="${frontUser.wxCode }" onblur="checkBankNum()" type="text" readonly="readonly">
				</div>
			</li>
			<%-- <li>
				<div class="txtDiv">
				...
				<div class="inpDiv">
					<input id="bankNameZhiInp" value="${frontUser.branch }" type="text" placeholder="请输入支行名称">
				</div>
			</li> --%>
		</ul>
		
		<a href="javascript:void(0)" tapmode onclick="saveBank()"
			class="aui-btn aui-btn-block comAuiBtn">修改绑定</a>
	</div>
	...
	<script type="text/javascript">
		...
		function saveBank(){
					...
					data:{			
					 ...
					 success:function(data){
						 layer.msg("绑定成功",{icon: 6,time:1000},function(){
								location.href = "indexs.htm?id="+$("#userId").val();
							});
					}, 
					error:function(data){
						layer.msg("绑定出错");
			...
			location.href = "updatebindWx.htm";
		}
	</script>
	<script type="text/javascript">
	function checkBankNum(){
		...
		if(userId != null && userId != ""){
			if(!reg.test(userId)){
				layer.tips("请输入正确的微信号", "#bankNumInp", {tips: [2, "#E34127"],time: 1000});
				falg = false;
			}
		}
		return falg;
