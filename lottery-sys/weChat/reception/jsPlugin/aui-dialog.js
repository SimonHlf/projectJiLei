...
(function( window, undefined ) {
    ...
    auiDialog.prototype = {
        ...
        create: function(params,callback) {
            ...
            console.log($(".aui-dialog-btn").length)
            if($(".aui-dialog-btn").length == 2){
            	$(".aui-dialog-btn:first-child").addClass("cancelColor");
            	$(".aui-dialog-btn:last-child").addClass("sureColor");
            }else if($(".aui-dialog-btn").length == 1){
            	$(".aui-dialog-btn:first-child").addClass("sureColor");
            }
...
//带有回调的dialog
function dialogCb(msg,btnVal1,btnVal2,fn){
	var dialog = new auiDialog({});
    dialog.alert({
        msg:msg,
        buttons:[btnVal1,btnVal2]
    },function(ret){
    	if(ret){
    		if(ret.buttonIndex == 1){//取消
    			return;
    		}else{
    			if(fn){
        			fn();
...
//一个按钮的回调
function dialogSingleCb(msg,fn){
	var dialog = new auiDialog({});
    dialog.alert({
        msg:msg,
        buttons:['确定']
