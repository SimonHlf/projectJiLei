...
<div class="widget model_widget">
	<div class="widget-body">
		<div id="horizontal-form">
			<form class="form-horizontal" id="editProductDetailsForm">
				...
				<div class="row">
					<p class="basicInfoTxt"><span></span>商品基本信息</p>
						...
						<div class="categoryDiv col-sm-4">
							...
							<%-- <a href="javascript:backEditCate('${product.id}')">&lt;&lt;返回修改</a> --%>
						</div>
					</div>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="name" name="name"
								placeholder="商品名" value="${product.name}" onblur="checkNameNum()" disabled>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="marketprice" name="marketprice"
								placeholder="市场价" type="number" value="${product.marketprice}" disabled>
							<span style="color:#2dc3e8">单位：元；市场价应高于下方商品规格中的最高价；</span>
						</div>
					</div>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="price" name="price"
								placeholder="现价" type="number" value="${product.price}"  disabled>
							<span style="color:#2dc3e8">单位：元</span>
						</div>
					</div>
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="costprice" name="costprice"
								placeholder="成本价" type="number" value="${product.costprice}" disabled>
							<span style="color:#2dc3e8">单位：元</span>
						</div>
					</div>
					<div class="comMargBot clearfix">
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="keywords" name="keywords"
								placeholder="用户搜索关键字" value="${product.keywords}" disabled>
						</div>
					</div>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<div class="checkbox" id="tagservice" name="${product.tagservice}">
								<v:AService aid="${product.tagservice}"></v:AService>
							</div>
						</div>
					</div>
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="unit" name="unit"
								placeholder="单位" value="${product.unit}" disabled>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="weight" name="weight"
								placeholder="重量" value="${product.weight}" type="number" disabled>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<input class="form-control" id="trans" name="trans"
								placeholder="运费" value="${product.trans}" type="number" disabled>
							<span style="color:#2dc3e8">单位：元</span>
						</div>
					</div>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-2">
							<input class="form-control" type="date" id="markettime" name="markettime"
								placeholder="上市时间" value="${product.markettime}" disabled>
						</div>
					</div>
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<select name="brandid" id="brandid">
								...
								<c:forEach var="b" items="${brands}">
									<c:if test="${product.brandid == b.id}">
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<select name="provinceid" id="provinceid">
								...
								<c:if test="${product.provinceid == r.regionid}">
										<option value="${r.regionid}" selected="selected">${r.regionname}</option>
									</c:if>
									<c:if test="${product.provinceid != r.regionid}">
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<v:items type="isreturned" id="isreturned" name="isreturned" value="${product.isreturned}"/>
						</div>
					</div>
					<div class="comMargBot clearfix">
						<label class="col-sm-2 control-label no-padding-right">是否可以兑换金币</label>
						<div class="col-sm-6">
							<v:items type="exchange" id="exchange" name="exchange" value="${product.exchange}"/>
							
						</div>
					</div>
					<div class="comMargBot clearfix">
						<label class="col-sm-2 control-label no-padding-right">兑换金币值</label>
						<div class="col-sm-6">
							<input class="form-control" id="gold" name="gold"
								placeholder="兑换金币值" value="${product.gold}" type="number" type="number" disabled>
							<span style="color:#2dc3e8">单位：金币</span>	
						</div>
					</div>
					<div class="comMargBot clearfix">
						<label class="col-sm-2 control-label no-padding-right">购买获得积分值</label>
						<div class="col-sm-6">
							<input class="form-control" id="attr0" name="attr0"
								placeholder="购买获得积分值" value="${product.attr0}" type="number" disabled>
							<span style="color:#2dc3e8">单位：积分</span>	
						</div>
					</div>
					...
					<div class="comMargBot clearfix">
						...
						<div class="col-sm-6">
							<textarea class="form-control" id="briefly" name="briefly" onblur="panduan()" disabled>${product.briefly}</textarea>
					...
					<div class="selProType comMargBot">
						<div class="comMargBot">
							...
							<div class="col-sm-6">
								<input type="hidden" id="proTypeInp" value="${product.attr1}"/>
								<label class="typeLabel" id="sing_sp" onclick="getProType(this)">
									单品
									<span class="radiusSp" attr="0"><em></em></span>
								</label>
								<label class="typeLabel" id="nosing_sp" onclick="getProType(this)">
									非单品
						...
						<div class="comMargBot clearfix">
							...
							<div class="col-sm-6">
								<input class="form-control" id="attr2" name="attr2"
									placeholder="库存" value="${product.attr2}" type="number" disabled>
					...
					<div class="noSinglePro clearfix" style="display:none;">
						<div class="comMargBot clearfix">
							...
							<div class="col-sm-6">
								<div class="checkbox" id="attrIds">
									<v:attribute cid="${product.categoryid}" aid="${product.attr1}"></v:attribute>
						...
						<div class="comMargBot clearfix col-sm-8" style="margin-left:16.6%;posiiton:relative;">
							<div class="addListTable">
								<%-- <a class="btn btn-success" onclick="addPAttrValues('${product.id}')">添加规格</a> --%>
							</div>
							<table class="table table-striped table-bordered">
								...
								<tbody id="pro_attrs">
									<c:forEach var="n" items="${listAttrs}">
										<tr>
											<td>${n.specName}</td>
											<td>${n.stock}</td>
											<td>${n.parentPrice}</td>
											<%-- <td>
												<a onclick="editProductAttr('${n.id}','${n.productId}')">修改</a>
												<a onclick="delProductAttr('${n.id}','${n.productId}')">删除</a>
											</td> --%>
										<tr>
									</c:forEach>
								</tbody>
					...
					<div class="comMargBot clearfix">
						<label class="col-sm-2 control-label no-padding-right">审核不通过原因</label>
						<div class="col-sm-6">
							<textarea class="form-control" id="message" name="message" onblur="panduan()">${product.message}</textarea>
							<span style="color:#2dc3e8">字数限制：不超过500字</span>	
						</div>
					</div>
				</div>
			</form>
			<div class="comMargBot clearfix saveProInfoBtn">
				<button class="btn btn-info" onclick="updateVerify(${product.id},1)">审核通过</button>
				<button class="btn btn-danger" onclick="updateVerify(${product.id},2)">审核不通过</button>
			</div>
		</div>
	</div>
</div>
<script>
	...
	$(function(){
		...
		if(proTypeInp=="0"){
			$("#sing_sp").find("span").addClass("selAct");
			$(".singlePro").show();
			$(".noSinglePro").hide();
		}else{
			$("#nosing_sp").find("span").addClass("selAct");
			$(".noSinglePro").show();
			$(".singlePro").hide();
		}
	});
    ...
    function panduan(){
    	//限制500字
    	 var introduction = $("#message").val();
    	if(introduction.length!=0){
    		if(introduction.length>500){
    			layer.msg("输入文字不能超过500个字");
    			$("#message").val(introduction.substring(0,500))
...
//保存商品信息
    function editProductDetails(){
    	...
    	$.ajax({
			method : 'post',
			url : 'auction/rmbProductController/editProductDetails.htm?attrIds='+attrIds+'&tids='+tids+'&tagservice='+tagservice+'&pictures1='+pictures1+'&pictures2='+pictures2,  
            contentType:false,
            processData:false,
			data : new FormData($("#editProductDetailsForm")[0]),
			success : function(data) {
