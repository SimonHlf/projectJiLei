	...
	<body>
		...
		<div class="confirmDivCon clearfix w1100">
			...
			<div class="confirmOrderDiv">
							...
							<li class="clearfix">
								<div class="comInnerCon confirWid3 detailCon">
									<div class="topProDetail">
										<div class="thumbnailImg">
											<img src="/${n.product.thumbnailurl}" />
										</div>
										<a class="pidName" href="" attrProId="${n.product.id}">${n.product.name}</a>
										<p>
											<a href="">
									...
									<c:if test="${n.product.attr1!='0'}">
										<input class="pavInp" type="hidden" value="${n.pav.id}">
										<div class="comInnerCon confirWid4">属性：${n.pav.specName}</div>
										<c:if test="${n.product.type==1 || n.product.type==4 }">
											<div class="comInnerCon confirWid4">单价：¥${n.pav.parentPrice}</div>
										</c:if>
										<c:if test="${n.product.type==2}">
											<div class="comInnerCon confirWid4">单价：${n.pav.parentPrice}金币</div>
										</c:if>
										<c:if test="${n.product.type==3}">
											<div class="comInnerCon confirWid4">单价：${n.pav.parentPrice}积分</div>
										</c:if>
									</c:if>
									<c:if test="${n.product.attr1=='0'}">
										<input class="pavInp" type="hidden" value="0">
										<div class="comInnerCon confirWid4">属性：无</div>
										<c:if test="${n.product.type==1 || n.product.type==4 }">
											<div class="comInnerCon confirWid4">单价：¥${n.product.price}</div>
										</c:if>
										<c:if test="${n.product.type==2}">
											<div class="comInnerCon confirWid4">单价：${n.product.price}金币</div>
										</c:if>
										<c:if test="${n.product.type==3}">
											<div class="comInnerCon confirWid4">单价：${n.product.price}积分</div>
										</c:if>
									</c:if>
								...
								<div class="comInnerCon confirWid4 clearfix">
									<div class="buyNum fl">
										...
										<p><input class="buyProNum" type="number" value="${n.num}" disabled></p>
									</div>
								</div>
								<c:if test="${type==1 || type==4 }">
									<div class="comInnerCon confirWid4">总价：¥<span class="totalPrice" id="totalPrice">${n.total}</span></div>
								</c:if>
								<c:if test="${type==2}">
									<div class="comInnerCon confirWid4">总价：<span class="totalPrice" id="totalPrice">${n.total}</span>金币</div>
								</c:if>
								<c:if test="${type==3}">
									<div class="comInnerCon confirWid4">总价：<span class="totalPrice" id="totalPrice">${n.total}</span>积分</div>
								</c:if>
				...
				<div class="payDiv fr">
					<div class="topPayDiv">
					 	<p><span>运费：${totalTrans}</span></p>
					 	
					 	<c:if test="${type==1 || product.type==4 }">
		...
		function getPidArray(){
			$(".pidName").each(function(i){
				pidArray.push($(".pidName").eq(i).attr("attrproid"));
			});
		}
		function getPavArray(){
			$(".pavInp").each(function(i){
				pavIdArray.push($(".pavInp").eq(i).val());
			});
		}
		function getProNum(){
			$(".buyProNum").each(function(i){
				proNumArray.push($(".buyProNum").eq(i).val());
			});
		}
