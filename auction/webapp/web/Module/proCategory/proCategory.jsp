...
<html>
	...
	<div class="categoryDiv w1100">
			...
			<dl class="clearfix">
				<input id="categroyInp5" type="hidden"/>
				<dt><span></span>分区</dt>
				<dd>
					<a href="javascript:void(0)" class="categroyAttr5" value="4">直购(送积分)</a>
					<a href="javascript:void(0)" class="categroyAttr5" value="2">金币直选</a>
					<a href="javascript:void(0)" class="categroyAttr5" value="1">直购(不送积分)</a>
					<a href="javascript:void(0)" class="categroyAttr5" value="3">积分专区</a>
				</dd>
			</dl>
			<dl class="clearfix">
	...
	<div class="maybeLike w1100" style="display:none">
		...
		<script type="text/javascript">
			...
			$(function(){
				...
				getCategory("categroyAttr5",'categroyInp5');
				getCategory("categroyAttr6",'categroyInp6');
				var str="";
				if(keywords!=""){
					str+='<a href="javascript:void(0)" value='+ keywords +' onclick=removeAttr("categroyAttr7")>'+
					'<span>'+keywords+'</span>'+
					'<em></em>'+
					'</a>';
				}
				if(cid1!=""){
					str+='<a href="javascript:void(0)" value='+ cid1 +' onclick=removeAttr("categroyAttr4")>'+
					'<span>'+cname1+'</span>'+
					'<em></em>'+
					'</a>';
				}
				if(cid2!=""){
					str+='<a href="javascript:void(0)" value='+ cid2 +' onclick=removeAttr("categroyAttr6")>'+
					'<span>'+cname2+'</span>'+
					'<em></em>'+
					'</a>';
				}
				if(type!=""){
					...
					str+='<a href="javascript:void(0)" value='+ type +' onclick=removeAttr("categroyAttr5")>'+
					'<span>'+typename+'</span>'+
					'<em></em>'+
					'</a>';
				}	
				...
				$(".selQuery").append(str);
				$(".queryCategDiv").show();
				if(keywords == "" && brandid =="" && weight == "" && price == "" && cid1 == "" && type == "" && cid2 == ""){
					$(".queryCategDiv").hide();
				}
			});
			//删除attr
			function removeAttr(options){
				if(options == 'categroyAttr1'){
					...
					cid1="";
					cname1="";
				}else if(options == 'categroyAttr5'){
					type="";
				}else if(options == 'categroyAttr6'){
					cid2="";
					cname2="";
				}else if(options == 'categroyAttr7'){
					keywords="";
				}
				window.location.href="${pageContext.request.contextPath }/web/shop/categoryApi/toCategoryJsp.htm?type="+type+"&brandid="+brandid+"&price="+price+"&weight="+weight+"&weightname="+weightname+"&pricename="+pricename+"&price_order="+price_order+"&cid1="+cid1+"&cid2="+cid2+"&keywords="+keywords+"&cmd=1";
			}
				...
				$("." + obj).each(function(i){
					$(this).click(function(){
						...
						if(obj=="categroyAttr4"){
							cid1=$(this).attr("value");
							cname1=$(this).html();    
						}
						if(obj=="categroyAttr5"){
			...
			function getMBList(page,pageSize){
				$.ajax({
					...
					url : "${pageContext.request.contextPath }/web/shop/categoryApi/toCategoryPage.htm?type="+type+"&brandid="+brandid+"&price="+price+"&weight="+weight+"&weightname="+weightname+"&pricename="+pricename+"&price_order="+price_order+"&cid1="+cid1+"&cid2="+cid2+"&keywords="+keywords,
