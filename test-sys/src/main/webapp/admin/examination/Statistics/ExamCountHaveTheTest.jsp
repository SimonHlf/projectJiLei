...
<%--</button>--%>
<button type="button" onclick="downloadHaveTheTest(${examId})" class="btn btn-primary">
        ...
        <table class="table table-bordered table-striped table-condensed flip-content">
            ...
            <c:forEach items="${list}" var="have">
                <tr>
                    ...
                    <td title="${have.attr5}">${have.paper_totail}</td>
                    <td title="${have.maxFen}">${have.maxFen}</td>
                    <td>
                        <c:choose>
                            <c:when test="${have.attr6 == 1}">
                                及格
                            </c:when>
                            <c:otherwise>
                                不及格
