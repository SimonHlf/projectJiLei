...
<body>
		...
		<div class="rightMainPart noFixed">
			...
			<div class="myExamResult">
				...
				<div class="myResult">
					...
					<p class="comSummaryP marginTopP">单选题：正确<span>${danxuanRighrNum}</span>道，错误<span>${danxuanWrongNum}</span>道</p>
					<p class="comSummaryP">多选题：正确<span>${duoxuanRightNum}</span>道，错误<span>${duoxuanWrongNum}</span>道</p>
					<p class="comSummaryP">判断题：正确<span>${panduanRightNum}</span>道，错误<span>${panduanWrongNum}</span>道</p>
					<div class="quesResultCard">
						<div class="succResult">
							...
							<div class="comQuesType singleTypeRes">
								...
								<ul class="clearfix">
									<%-- <c:if test="${duoxuanRightList!=null && duoxuanRightList!=''}"> --%>
										<c:forEach items="${duoxuanRightList}" var="duoxuanRight">
											<li class="active"><a href="javascript:void(0)">${duoxuanRight}</a></li>
										</c:forEach>
									<%-- </c:if> --%>
									<%-- <c:if test="${duoxuanRightList.length==0}"> --%>
										<!-- <li class="active">你的多选题没有正确的</li> -->
									<%-- </c:if> --%>
							...
							<div class="comQuesType singleTypeRes">
								...
								<ul class="clearfix">
									<%-- <c:if test="${panduanRightList!=null && panduanRightList!=''}"> --%>
										<c:forEach items="${panduanRightList}" var="panduanRight">
											<li class="active"><a href="javascript:void(0)">${panduanRight}</a></li>
										</c:forEach>
									<%-- </c:if> --%>
									<%-- <c:if test="${panduanRightList.length==0}">
										<li class="active">你的判断题没有正确的</li>
									</c:if> --%>
						...
						<div class="errResult">
							...
							<div class="comQuesType singleTypeRes">
								<p class="comTypeTit">单选题</p>
								<ul class="clearfix">
									<%-- <c:if test="${danxuanWrongList!=null && danxuanWrongList!=''}"> --%>
										<c:forEach items="${danxuanWrongList}" var="danxuanWrong">
											<li class="err"><a href="javascript:void(0)">${danxuanWrong}</a></li>
										</c:forEach>
									<%-- </c:if> --%>
									<%-- <c:if test="${danxuanWrongList.length==0}"> --%>
										<li class="active">你的单选题没有错误的</li>
									<%-- </c:if> --%>
							...
							<div class="comQuesType singleTypeRes">
								<p class="comTypeTit">多选题</p>
								<ul class="clearfix">
									<%-- <c:if test="${duoxuanWrongList!=null && duoxuanWrongList!=''}"> --%>
										<c:forEach items="${duoxuanWrongList }" var="duoxuanWrong">
										<li class="err"><a href="javascript:void(0)">${duoxuanWrong }</a></li>
									</c:forEach>	
									<%-- </c:if> --%>
									<%-- <c:if test="${duoxuanWrongList.length==0}">
										<li class="active">你的多选题没有错误的</li>
									</c:if> --%>
							...
							<div class="comQuesType singleTypeRes">
								<p class="comTypeTit">判断题</p>
								<ul class="clearfix">
									<%-- <c:if test="${panduanWrongList!=null && panduanWrongList!=''}"> --%>
										<c:forEach items="${panduanWrongList }" var="panduanWrong">
											<li class="err"><a href="javascript:void(0)">${panduanWrong }</a></li>
										</c:forEach>
									<%-- </c:if> --%>
									<%-- <c:if test="${panduanWrongList.length==0}">
										<li class="active">你的判断题没有错误</li>
									</c:if> --%>
								</ul>
							</div>
						</div>
	...
	<div class="handPaper w1100">
		<a class="backPerBtn" onclick="jumpCen()" href="#">返回个人中心</a>
	</div>
